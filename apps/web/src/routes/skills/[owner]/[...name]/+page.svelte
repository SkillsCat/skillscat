...
{#if data.skill}
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-4 sm:pb-8 skill-detail-container">
    ...
    <nav class="breadcrumb">
      <ol>
        <li class="breadcrumb-fixed hide-mobile"><a href={i18n.href('/')}>{copy.breadcrumbHome}</a></li>
        <li class="breadcrumb-sep hide-mobile">/</li>
        <li class="breadcrumb-fixed"><a href={i18n.href('/trending')}>{copy.breadcrumbSkills}</a></li>
        <li class="breadcrumb-sep">/</li>
        <li class="breadcrumb-truncate">
          <a
            href={i18n.href(getAuthorProfileUrl())}
          >
    ...
    {#snippet renderActionButtons()}
      ...
      <div class="action-buttons">
        ...
        {#if githubUrl}
          <a href={i18n.href(githubUrl)} target="_blank" rel="noopener noreferrer" class="github-btn action-btn">
        ...
        <div class="card skill-header">
          ...
          {#if shouldShowDerivedFromCallout}
            <div class="mt-5 rounded-2xl border border-amber-200 bg-amber-50/90 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-100">
              ...
              <a
                href={i18n.href(originSkillHref)}
          ...
          <div class="skill-meta">
            ...
            {:else if data.skill.authorUsername}
              <a
                href={i18n.href(`/u/${encodeURIComponent(data.skill.authorUsername)}`)}
                class="skill-meta-item"
              >
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            ...
            {:else if data.skill.repoOwner}
              <a
                href={i18n.href(`/u/${encodeURIComponent(data.skill.repoOwner)}`)}
                class="skill-meta-item"
              >
            ...
            {#if displayRecommendSkills.length > 0}
              <div class="space-y-3">
                {#each displayRecommendSkills as recommendSkill (recommendSkill.id)}
                  ...
                  <a
                    href={i18n.href(getRecommendSkillHref(recommendSkill.slug))}

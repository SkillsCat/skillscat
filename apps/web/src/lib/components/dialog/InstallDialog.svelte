  ...
  async function handleInstall() {
    ...
    try {
      ...
      if (typeof pickerWindow.showDirectoryPicker !== 'function') {
        installError = messages.installDialog.fileSystemUnsupported;
        return;
      }
      ...
      if (!response.ok) {
        throw new Error(messages.installDialog.failedToFetchSkillFiles);
      }
      ...
      installError = errorValue instanceof Error && errorValue.message
        ? errorValue.message
        : messages.installDialog.installationFailed;
    } finally {
      isInstalling = false;
    }
  }
        ...
        {#if open}
          <div {...props} class="dialog" transition:fly={{ y: 10, duration: 200 }}>
            <div class="dialog-header">
              <Dialog.Title class="dialog-title">{i18n.t(messages.installDialog.title, { skillName })}</Dialog.Title>
            ...
            <div class="dialog-content">
              <!-- Target Selection -->
              <div class="section">
                <span class="section-label">{messages.installDialog.installFor}</span>
                ...
                <div class="section">
                  ...
                  >
                    {#if isInstalling}
                      ...
                      {messages.installDialog.installToLocalDirectoryBusy}
                    {:else}
                      <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                      </svg>
                      {messages.installDialog.installToLocalDirectory}
                    {/if}
                  </Button>
                  {#if installError}
              ...
              <!-- Command Copy -->
              <div class="section">
                <span class="section-label">{messages.installDialog.orCopyCommand}</span>

                {#if platform === 'windows'}
                  <div class="command-box">
                    <div class="command-label">{messages.installDialog.powershell}</div>
              ...
              {#if !isMobile}
                <div class="section">
                  ...
                  >
                    ...
                    {messages.installDialog.downloadAsZip}
                  </a>
                </div>
              {/if}
              ...
              <div class="section github-section">
                <p class="github-text">
                  {messages.installDialog.likeThisSkill}
                </p>
                <a
                ...
                >
                  ...
                  {messages.installDialog.starOnGitHub}
                </a>
              </div>
            </div>
          </div>
